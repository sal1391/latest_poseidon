...
``query_builder`` import, no LLM. ``build_brief_pdf`` (Task 4) renders the
brief to PDF and uploads it via ``ArtifactStore``. Each tool is unit-tested
in ``tests/`` on its own, ahead of the skill that will call them in
sequence.
"""
