...
- Customer resolution tiers and thresholds are law: exact (casefold) → token-set → fuzzy; `>= 0.80` auto-apply, `0.60–0.80` → clarification candidates (each individually >= 0.60; max 3, scored order), `< 0.60` → no match. Thresholds are named constants; rapidfuzz `token_set_ratio`/100.0.
...
- Offline-by-default: core suites use in-test fake DataClients with fixed dimension/period data; pg-marked goldens resolve against the seeded db (misspelled "Northstar Linez" → auto-apply; "Meridiann" → multi-candidate clarification since seed has several Meridian * — see the line-191 amendment: bare "meridian" auto-applies live; etc.).
...
`parse_turn` order: normalize → detect customer phrase (heuristic: quoted spans, "for/about/on <TitleCase run>", or slot carry) → resolve customer (and port when a port cue like "port of X"/"at X" matches a LOC_NM value) → mask each AUTO-APPLIED resolution's source span from the working text before period parsing (replace the span with spaces so all other offsets stay stable; mask only auto-applied resolutions — tier exact/token, or fuzzy >= 0.80 — never candidate-band or unknown, which produce issues instead) so month-named customers ("May Shipping") cannot be re-read as period phrases; documented residual: an AMBIGUOUS month-named mention can still shadow-parse as a period, acceptable because the turn already carries a customer_ambiguous issue — a clarification turn, never a silent answer → parse periods (with `data.available_periods(entity)`, over the masked text) → apply carry (updates derived from resolutions: resolved value → replace; explicit "clear"/"reset" phrase → None; no mention → UNSET) → hints → assemble ParsedTurn with all issues collected. Phrase-detection heuristics must be conservative: an unmatched TitleCase run produces NO issue unless a customer cue-word was present (avoid false "unknown customer" on ordinary sentences) — pin both directions.
...
- [ ] Tests FIRST: hinter table (weights/order/empty); pipeline offline with fake DataClient (fixed 6 customers incl. the demo trio AND one month-named customer "May Shipping", fixed available range): the flagship case — message "Top GP customers for Port of Singapore in April 2026" + empty slots → period_a April, customer None (no cue), port Singapore resolved, hints lead with data_qa.metric_query, zero issues; masking case — "gp for May Shipping in april 2026" + empty slots → customer resolves to May Shipping, period_a = April 2026 (NOT May), zero issues; masking counter-case — "gp for may 2026" (no TitleCase run, no customer cue beyond the preposition) → no customer detection, period_a = May 2026 via month-year, zero issues; carry case — prior slots customer Northstar + message "and for May?" → customer carried, period replaced; clarification case; unavailable-period case. `@pytest.mark.pg` goldens (established skip pattern): resolver against live seeded CUST_NM values ("Northstar Linez" auto-applies; "Meridiann" yields the multi-Meridian candidate band — see the line-191 amendment: bare "meridian" auto-applies live) and full `parse_turn` with `SyntheticDataClient`. RED → implement → GREEN offline + pg → ruff. **Commit** — `feat(parsing): skill hinter and parse_turn pipeline with live resolution goldens`
...
- Deferred (doc 02 §5 "exact/alias" tier 1): the alias tier ships as exact-only — no alias data exists in the repo, so a lookup against an empty table would be dead code. Recorded in `customer_resolver.py`'s docstring as a certification concern (it must land before real customer data does).
- Deferred (doc 02 §5 hinter output "+ mode hints"): shipped `hint()` folds MODE_HINTS into the skill scores and returns only `CandidateSkill` tuples; `ParsedTurn` carries no mode field. A separate mode-hint output waits for Phase 6, which owns mode end to end. Recorded in `lexicon.py` where MODE_HINTS is documented.
