...
- app.py wiring (AMENDED post-T2: a REAL Perplexity key turned out to exist in the ambient environment, so key-presence is the WRONG gate — it would silently burn API credits on demo turns): the fixture-vs-real decision follows `settings.llm_mode` — `"stub"` → ToolServerRegistry override installs a FixtureResearchTool (reads the clean fixture; transport "fixture"); `"live"` → the registry resolves per TOOL_TRANSPORT_PERPLEXITY as designed. ONE mode switch governs all external calls (LLM and research move together — coherent with P6's "stub LLM mode throughout"); boot log line discloses which transport is active. AMENDED also: `ResearchResult` gains an additive defaulted field `summary: str = ""` (T1 dataclass, frozen-compatible — sanctioned) so the schema's summary threads through to the skill's text part (T2 flagged the gap).
