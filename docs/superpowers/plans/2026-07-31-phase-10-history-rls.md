...
- **Runtime enforcement on privileged DSNs (amended after T1's discovery):** the compose `poseidon` role is a Postgres SUPERUSER, which no policy or `FORCE` clause can bind — so `rls_transaction` ALSO executes `SET LOCAL ROLE poseidon_app` immediately after `set_config`, making RLS enforcement real for the actual runtime connection even on a privileged DSN. Transaction-scoped like the identity context itself. Configurable via `Settings.database_app_role: str | None` (default `"poseidon_app"`; `None` disables the SET ROLE for deploy DSNs that already connect as a non-privileged role without membership). This keeps doc 05's "the application connects as a role with no BYPASSRLS" true in every environment, and it is the mechanism that lets Task 2's WHERE-clause-free store tests pass locally. `core/config.py` (+`database_app_role`) is added to the sanctioned-modifications list for Task 1.
...
**Interfaces consumed:** Task 2's exactly. **Produced for Task 4:** `GET /api/conversations?limit=&cursor=` → `{"items": [...], "next_cursor": str|null}` (each item carries `id` + `title` — **amended after T3: `mode`/`updated_at` are deferred until a consumer exists; adding them requires reopening Task 2's closed `list_conversations` review for fields Task 4 does not read, and the cursor is server-encoded so the client never needs `updated_at`**); `GET /api/conversations/{cid}/messages?limit=&cursor=` → same envelope; the `done` SSE event payload gains an additive `"title": str|null` field (null except the event that first sets it).
...
- [ ] **Step 3:** `app.py`: build ONE `Engine` via `db.build_engine(settings.database_url)`; pass it to `RunLogWriter` AND `HistoryStore` (delete the second `create_engine` — one pool per process; health.py's throwaway probe engine stays as-is). `app.state.history_store = HistoryStore(engine, app_role=settings.database_app_role)` — **the `app_role` argument is load-bearing (amended after T2's review): omitting it silently disables RLS enforcement on the dev superuser DSN, the exact bypass class T2 closed**; delete `transcript_store`/`conversation_state_store` wiring.
- [ ] **Step 4:** `live_chat.py`: routes resolve `user_history = request.app.state.history_store.for_user(request.state.user.sub)` per request. List/messages gain the pagination params + envelope; **a malformed `cursor` query param maps T2's typed cursor exception to a 400 RFC-7807 problem detail (amended: T2's fix round adds the typed exception — do not let it escape as a 500)**. Send route: `append_user_message` at accept; `TurnTranscriptBuffer` folds frames exactly where `_record_transcript_frame` folds today; on terminal frame, `write_assistant_message`; **`turn_id` minting switches `uuid.uuid4()` → `uuid7()` (amended: it lands in a real uuid column now; uuid7 is the phase's only id mint)**; orchestrator receives `DbStateStore(user_history)` — signature-identical, zero orchestrator edits. After a SUCCESSFUL first turn (`turn_index == 1`, status ok), call `title_for(question, role_client, prompt_registry)`; on `""` fall back to `question[:60]`; `set_title`, and include it in that turn's `done` payload (additive field only — every existing done assertion must pass unchanged). Feedback routes → `FeedbackStubStore`, **plus an existence gate (amended): the old TranscriptStore `_messages` membership check is gone — before upsert/get, verify the message id is visible via `UserHistory` (an RLS-filtered SELECT) and 404 when it is not, preserving today's 404-on-unknown-mid contract**. Delete `TranscriptStore` (class + tests move/port per Task 2 step 1).
