...
1. localhost:5173 → click "Existing customer" → asked which customer → type "Northstar Lines" → watch the brief stream: metric cards, top-ports table, three phase sections (stub-labeled narrative), PDF card (or its honest skip line) → then ask a data question and a news question — both still work with the customer carried.
2. New chat → "New prospect" → type any company → research-first brief per D10.
3. `pytest tests/test_chat_e2e_scripted.py -m pg -v` → both flow scripts green incl. run-log row inspection.
