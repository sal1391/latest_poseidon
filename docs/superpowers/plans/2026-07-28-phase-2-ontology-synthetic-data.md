...
Three things Phase 2 settled locally that a later phase inherits rather than re-decides.

**Ontology path resolution is compose-shaped, not image-shaped.** `poseidon.core.ontology.loader` anchors `DEFAULT_ONTOLOGY_PATH` at `parents[4] / "ontology" / "ontology.yml"` — a fixed offset from the module file to the repo root. That resolves in-container today only because `infra/docker-compose.yml` bind-mounts the repo's `/ontology` directory into the backend service; nothing in the image itself carries the ontology. A production image must therefore either COPY `ontology/` in during the build or support a `POSEIDON_ONTOLOGY_PATH` environment override that `load()` consults ahead of the anchored default (`load(path=...)` already takes the argument; only the env plumbing is missing). Which of the two is left to the deploy phases — the tradeoff is image immutability versus refreshing a certified ontology without a rebuild, and Phase 2 has no stake in the answer.

**`PeriodRange.end` and `PeriodWindow.end` do NOT share a convention.** `PeriodRange` (a query *result*: MIN/MAX of the entity's date column) has an **inclusive** `end` — the newest date actually present. `PeriodWindow` (a query *input*) is half-open, `[start, end)`. Feeding a `PeriodRange` straight into a `PeriodWindow` silently drops the last day of data, and a single-day range raises (`PeriodWindow` now rejects `start >= end`). Any skill that turns "what periods exist?" into "query that whole range" must add a day: `PeriodWindow(r.start, r.end + timedelta(days=1))`. Both sides are documented at their definitions; unifying them is a bigger change than Phase 2 should make unilaterally.

**The postgres dialect is correct against the synthetic store only.** `query_builder`'s `"postgres"` branch targets the local `synthetic` schema, whose `PERIOD_DATE` is a real `date` column (migration 0002's documented, deliberate deviation) — so it compares the bare column. The certified GL table declares `PERIOD_DATE` as VARCHAR, which is why the `"snowflake"` branch wraps it in `TO_DATE(...)`. Pointing the postgres dialect at any other Postgres store that mirrors the certified VARCHAR typing would produce wrong or failing comparisons; the dialect is not a general "Postgres" renderer, it is the renderer for *this* schema.
