...
- **Phase 6 (real chat end to end): ALL 5 TASKS COMPLETE and THE CUTOVER IS LIVE** — final whole-branch review in progress at the stamp below. **Heads up before you open the app: localhost:5173 is no longer the mock.** Typing a question now runs the real pipeline — deterministic parse → fuzzy resolver → certified SQL against the seeded synthetic Postgres → streamed table + proof parts — and every turn is recorded in the new run-log tables (`turn_run`/`llm_calls`/`tool_calls`). No AWS keys involved: a deterministic dev router stands in for the LLM (it reads the same rendered context a real model would), so the demo answers any reasonable metric question offline. Your 4-turn gate script is in `infra/runbooks/local.md` (use its exact phrasings — capital-M "Meridiann" matters, the customer detector requires TitleCase like a real name would have). Suites: 999 offline + 41 live-Postgres + 28 frontend, zero failures. Playwright screenshots of the live browser run are in the Phase 6 workspace folder.

**As of: evening — Phases 2, 3, 4, AND 5 ALL COMPLETE and frozen (`34ea4c1`). Four phases built through the full loop each: per-task reviews, an Opus whole-branch final review, a fix wave, a clean re-verification. Final suites: 885 offline + 31 live-Postgres, lint clean, live-LLM suite armed and waiting only on your AWS keys. The deterministic core (parse → resolve → mask), the skill machinery, and the LLM layer (providers, prompts, agent loop, router-decision tests) are all in place — Phase 6 (wiring real chat end-to-end over all of it) is the next unit of work; its plan keeps the mock chat intact until the real path passes its gates, so localhost:5173 stays demo-able throughout. Phase 4 highlights stand: the "may" false positive is gated, clarification chips only show plausible names (≥ 0.60), customer names are masked before period parsing, and a follow-up turn can never silently inherit a comparison you didn't re-ask for. Merge order recommendation unchanged: pass Phase 2 → merge it → then review `phase-3-8-overnight` (Phases 3-5) as one unit. Your decision pile: 6 items, all listed above with context.**
