...
/**
 * Runs `fn` inside a transaction whose Postgres GUC `app.user_sub` is set to
 * `sub`, which is what the RLS policies read (decision D28).
 *
 ...
 * the parameter exists to prevent. `SET LOCAL ROLE poseidon_app` drops to a
 * non-owner, non-BYPASSRLS role so a forgotten filter returns zero foreign
...
export async function withUser<T>(
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.execute(sql`SET LOCAL ROLE poseidon_app`);
