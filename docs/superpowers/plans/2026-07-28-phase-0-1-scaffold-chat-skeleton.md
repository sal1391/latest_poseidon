...
- Produces Zustand store `useChatStore` with state `{ conversations: Conversation[]; activeId: string | null; messages: Record<string, Message[]>; streamingByConv: Record<string, boolean>; feedback: Record<string, { verdict: "up" | "down"; comment?: string }> }` and actions:
  - `bootstrap(): Promise<void>` — list conversations; if empty, create one (stores opener); if non-empty, open the newest. Re-entrant: concurrent calls share one in-flight promise (memoized), so StrictMode double-invocation cannot create two conversations.
  - `newConversation(): Promise<string>` — PREPENDS to `conversations` (the list is newest-first, matching the API); `openConversation(cid): Promise<void>` (loads messages via `getMessages`).
  - `sendMessage(cid, text): Promise<void>` — no-ops if a turn is already streaming for that cid (per-conversation reentrancy guard); otherwise appends user message locally, sets streaming, calls `streamTurn` wiring events to `applyEvent`, clears streaming on completion or thrown error (thrown → append synthetic error part). The UI additionally disables the composer the instant a send is issued (a local `sending` flag set before any await), so the guard is belt-and-braces.
