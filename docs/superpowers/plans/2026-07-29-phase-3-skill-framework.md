...
- `skill.run(ctx, args)`: empty result (all-None metric values or zero breakdown rows) → `ok=True` with `text_part("No data for this selection.")` + proof stating `Rows: 0` (doc 02 §6a — never a hallucinated narrative). Happy path → parts + proof. `SpecValidationError` → structured 422 as above.
