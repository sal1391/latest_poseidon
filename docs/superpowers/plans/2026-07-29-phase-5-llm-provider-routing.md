...
- Do not modify Phase 2-4 modules, mock_chat, dev_runner, frontend, legacy root, or docs/architecture. Additive only: `core/config.py` Settings fields, `pyproject.toml` deps + marker. AMENDED (post-T3 review): plus ONE additive discovery-time validation in `core/skills/registry.py` — the Bedrock-safe tool-name check (every registered skill id must map injectively under "." → "__" and stay within ToolName's 64-char cap; violation raises SkillDefinitionError at discovery). Sanctioned because the registry is where ids are minted and its fail-fast philosophy ("wrong at start-up, in one legible line") is the correct home for the invariant the Bedrock boundary translation relies on; a collision would otherwise dispatch to the wrong REAL skill silently. The translation itself stays confined to bedrock.py as a pure forward/reverse function pair used at FOUR sites (RE-AMENDED post-T4: toolConfig build + the assistant toolUse message echo, both forward; both tool_use normalization sites, reverse — the echo site was discovered by T4: ToolUseBlock.name is the same ToolName shape and Converse requires the echo before toolResult, so from loop iteration 2 a dotted name would fail server-side); the reverse map is unconditional-mechanical (hallucinated names fail at dispatch's existing 404, never in the provider).
