...
- Sanctioned modifications to closed files: `api/live_chat.py` (feedback routes swap stub→store + the 422 pin; **amended after T1 review: + `send_message`'s `_persist_once` threading `TurnOutcome.turn_run_id` into the persisted `messages.turn_id` column — a replayed message previously carried a freshly-minted, orphaned `turn_id`, which is unsatisfiable against the new `message_feedback.run_id` NOT NULL FK on a replayed turn's feedback; the T1 reviewer independently re-derived both of the implementer's claims from code outside the diff — the change is a byte-for-byte no-op on every non-replay path (`orchestrator.py`'s `_begin_turn` already passes `turn_run_id=sink.turn_id`, and `RunLogWriter.start_turn` uses a given id verbatim), and no production code read `messages.turn_id` before this task (grepped `history.py`'s SELECT sites) — so the fix is ratified in scope here rather than reverted or re-dispatched**), `core/chat/history.py` ONLY for deleting `FeedbackStubStore` (the stub lives there since P10 — amended pre-dispatch by the new sanction-line completeness check, which caught this line originally claiming history.py untouched while Task 1's own step deletes the stub); `poseidon/scripts/export_router_cases.py` (the thumbs-down-first extension — reopened with its own review); frontend `features/*` + `state/chatStore.ts` + `api/client.ts` (thumbs live + a11y + the two UX gaps); `mocks/handlers.ts`. Everything else untouched.
