...
| D46 | **Prompts live in one canonical top-level `prompts/` tree** with XML system-prompt structure. Skills stop owning prompt files and resolve them by id through `PromptRegistry`. A documented exception to the co-location folder law, taken so prompts stay editable in one place. (Codex Q2) |
| D47 | **Ad-hoc in-chat report generation is open to every `Poseidon:Sales` user.** `Poseidon:ReportAdmin` gates only the shared artifacts — saved definitions, stored runs, and sends. Enforced at dispatch, not by hiding chips. Rationale: D44 already makes the whole book visible to all Sales users, so generating a view of data they can already see adds no exposure; what needs protecting is what other people receive. (Codex Q3) |
| D48 | **A definition's scope is immutable once it has runs.** `slice_column` and `slice_value` cannot change after the first run; name, recipients and `enabled` stay editable. Changing scope requires a new definition. Resolves half of C05 — a queued run can no longer generate an office nobody asked for. (Codex Q4) |
| D49 | **A report link opens the run it names**, banner-marked as superseded when a newer run exists, with one click to the latest. **The grounded chat is scoped to whatever run the panel shows**, so panel and chat agree by construction rather than by reconciliation. Revises the "chat always follows the current run" policy and resolves C06. (Codex Q5) |
| D50 | **Saved and emailed reports run on complete months only**, and refuse with a stated reason when the prior month is absent or empty. **Day-matched partial-month comparison is available in ad-hoc chat only** (e.g. Sept 1-7 vs Aug 1-7) and is explicitly outside the mom-comparison parity scope. Requires `DataClient.available_periods` to enumerate the months actually present, not return a MIN/MAX range. (Codex Q6) |
| D51 | **The reconciliation baseline runs mom-comparison with the same row filter as Poseidon**, so the parity test proves the maths and isolates it from the separate question of which rows belong. A filter difference then becomes a known, explained difference rather than a failing test. (Codex Q7) |
| D52 | **Ranked tables carry an `Other (N customers)` row** so the column visibly sums to the footer, and the footer equals the whole-office KPI (§4.4). A rounding footnote appears only when display rounding makes the column differ from the footer by one currency unit. (Codex Q8) |
| D53 | **`report_lookup` takes explicit `limit` and `offset`**, defaulted to fit the 4,000-character tool-result cap (`core/llm/loop.py:126`), and returns an explicit "N rows remaining" marker. The cap is enforced by construction — never silent truncation. (Codex Q9; resolved by Claude as a code-level contract.) |
| D54 | **All queries in one report run share a snapshot** where the backend supports it — a single `REPEATABLE READ` transaction on Postgres. Where it cannot be guaranteed (the Snowflake handoff must confirm the view's capability), the existing reconciliation control detects drift and the run retries once before erroring, rather than reporting figures assembled from two different states. (Codex Q10; resolved by Claude as a code-level contract.) |

> **D46-D54 added 2026-09-07** from the owner's answers to the Codex review's questions.
> D48/D49/D50 change contracts elsewhere in this document that have not yet been rewritten to
> match; see `2026-09-07-codex-findings-triage.md` and the migration design's phase plan.
